...
%\item<2> Une variable peut changer de type au cours de l'exécution
%\end{itemize}
 %\pause 
	%\begin{remarque}
    %\begin{itemize}
...
%\only<1->{
%\begin{itemize}
%\item<1-> \textbf{Les types intégrés de Python}
%\begin{itemize}
    %\item<2-> Entiers signés (complément à deux) : le type \emph{int} 
    %\item<2-> Réels (IEEE 754) : le type \emph{float} en double précision (64 bits)
    %\item<2-> Complexes : le type \emph{complex}
		%\item<3-> Booléens : le type \emph{bool}
		%\item<4-> Chaînes de caractères : type \emph{str}

...
\begin{frame}[fragile]
 \frametitle{Argument(s) par défaut}
...
\begin{remarque}
  ...
  \begin{semiverbatim}
	...
	lst=aj(5)   \# \only<1>{lst = ?}\only<2-3>{lst = [1, 2, 5]}
	lst1=aj(6)  \# \only<1-2>{ lst1 = ?, lst = ?} \only<3>{lst1 = lst = [1, 2, 5, 6]}
	\end{semiverbatim}
...
\end{itemize}
%{\small
  \begin{exemple}[Passage par valeur/variable]<4-6>
    \only<4-6>{
		\begin{semiverbatim}
		\vspace{-0.5cm}
	...
	print(el)  \textcolor[rgb]{0.24,0.7,0.44}{\# => el = \only<5-6> {'deux' }}
	
	print(lst) \textcolor[rgb]{0.24,0.7,0.44}{\# => lst = \only<6> {['un', 'deux']}}
	\end{semiverbatim}
	}
   \end{exemple}
...
\begin{frame}[fragile]
  \frametitle{Passage par variable}
  ...
  \begin{exemple}[Passage par variable : type modifiable]
	...
	lst=['un'] ; el='deux'
	ajoutsupp(lst, el)
	print(lst) \textcolor[rgb]{0.24,0.7,0.44}{\# => lst = ?}
	ajoutsupp1(lst, el)
	print(lst)  \textcolor[rgb]{0.24,0.7,0.44}{\# => lst = ? }
	}
	\end{semiverbatim}
...
\begin{frame}
  \frametitle{Portée de variables}
	 \only<1>{
	\begin{defi}
  ...
  \frametitle{Variables globales/locales}
	...
	\item La recherche d'un nom débute dans l'espace de noms le plus intérieur, puis s'étend jusqu'à l'espace de nom global
  	\begin{itemize}
		...
		\item Le contenu d'une variable golobale est modifiable si elle est mutable (modifiable).
...
\begin{frame}
  \frametitle{Modules, bibiothèques, librairies}
...
{\small
    \begin{semiverbatim}
		\vspace{-.5cm}
		>>> import sys \only<7>{as alias}
		
		>>> \only<6>{sys.path}\only<7>{alias.path}
		
		['','C:\\python-3.3.5.amd64\\Lib\\idlelib','C:\\python-
		3.3.5.amd64\\lib\\site-packages\\Pythonwin', \dots]
		
	  >>> \only<6>{sys.path}\only<7>{alias.path}.append('chemin de module')	
 \end{semiverbatim}
}
  \end{exemple}
...
\begin{remarques}
	...
	\begin{itemize}
		\item Utilisation  corps principal du programme via la variable \verb!\_\kern-.14ex\_name\_\kern-.14ex\_ 
...
\begin{frame} [fragile]
  \frametitle{Utilisation de modules}
...
$ python3 mon_module.py   # => affiche ce qui suit :

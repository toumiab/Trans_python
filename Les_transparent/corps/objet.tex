    ...
    \begin{exemple}[Figures]
      ...
      \begin{semiverbatim}
        ...
        \hspace{1cm}\textcolor{red}{self.\_ordonnee} = y
				...
				\uncover<3>{class Cercle \textcolor{red}{(Figure)}:
   ...
   \small
      \begin{semiverbatim}
        ...
        \hspace{1cm}\textcolor<4>{red}{super()}.\textcolor<6-7>{red}{\_\_init\_\_(x,y)} \textcolor[rgb]{0,0.58,0}{\# définit et initialise }
				...
				c = \textcolor<2>{red}{Cercle(10, 3, 2)} \textcolor{green}{\# création de l'objet f1}
				}
...
\begin{frame}
  \frametitle{Principe}
  \begin{itemize}
  \item Plusieurs méthodes avec le même nom dans un
...
\begin{frame}
  \frametitle{Exemple de surcharge : méthode \_\_str\_\_}
 \begin{itemize}
  ...
  \item Prédéfinie dans la classe \texttt{object} 
	...
	Exemple vu (le type Pile) : l'affichage d'un objet \texttt{Pile}\pause \\
  ...
  \begin{semiverbatim}
    ...
    \only<-5>{\small \verb~<\_\_main\_\_.Pile object at 0x000000000575DBA8>~}\only<6->{Fig. Pos = (3,5)}
...
\begin{frame}
  \frametitle{Quelques méthodes à surcharger}
			\begin{center}
  ...
  \begin{exemple}
    ...
    \begin{semiverbatim}
      ...
      \hspace{.5cm}def \_\_init\_\_(self, x, y):
			...
			\only<3->{\hspace{1cm} \textcolor<3>{red}{Figure.nb\_figure += 1}}
      
			\only<3->{\hspace{.5cm}\textcolor[rgb]{0.8,0.6,1}{@classmethod}
			
			\hspace{.5cm}def nombre\_figures(\textcolor[rgb]{1,0,0}{cls}):
		
      \hspace{1.5cm}print("nb de figures crées:"'+str(Figure.nb\_figure)))}

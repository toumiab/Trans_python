...
%\vspace{-1 cm}
%\begin{figure}
		%\includegraphics[height=8cm, width=11cm]{images/joie.png}
		%\label{fig:joie}
%\end{figure}
%\end{frame}
\section{Rappel : Objet }

\subsection {Type abstrait de données}

\begin{frame}

  \frametitle{Exemple : rationnels }%\chrono}
\begin{itemize}
\item Définition d'une Classe : Constructeur et Variables d'instance
\item + méthodes d'instance : égalité, multiplication, représentation, ... 

 \end{itemize}

  \begin{semiverbatim}
    \textcolor{red}{class Rationnel (object):}
\only<1-4>{
...
\only<2>{
  ...
  \hspace{1cm}def \textcolor<2>{red}{\_\_eq\_\_}(\textcolor{blue}{self}, other):
...
\only<3>{
	...
	\hspace{1cm}def \textcolor<3>{red}{\_\_mult\_\_}(\textcolor{blue}{self}, other):
...
\hspace{1cm}def \textcolor<4>{red}{\_\_str\_\_}(\textcolor{blue}{self}):
...
\begin{frame}
  \frametitle{Exemple : Classe/Type \texttt{ Rationnel} }%\chrono}
  ...
  \frametitle{Définition}
	\begin{remarques}[Quelques remarques importantes ]
		...
		\begin{itemize}
			...
			\item Les attributs (variables d'instance) sont \textit{généralement} créés  et initialisés dans le constructeur \\
			(exemple : \textcolor{blue}{$self.\_\_nomVar$, $self.\_nomVar$, $self.nomVar$})
...
\subsection{Notion d'objet :Concepts fondamentaux}
...
\begin{frame}
  ...
  \begin{itemize}
  \item {\color{red}encapsulation} : les décorateurs ; les getteurs/setteurs
  \item héritage (UE 2.1)
  \item polymorphisme (UE 2.1)
  \end{itemize}

    \begin{defi}[Encapsulation]
      Mécanisme permettent de cacher l'implantation d'un TAD. Ce méchanisme nous fournit une
      interface pour accéder aux données (lecture/écriture de contenu).
    \end{defi}
    ...
    \begin{exemple}[Figures]
      ...
      \begin{semiverbatim}
        ...
        \hspace{1cm}\textcolor{red}{self.\_ordonnee} = y
				...
				f1 = Figure(10,2) \textcolor{green}{\# création de l'objet  Figure f1}
				
				f2 = Figure(0, 4) \textcolor{green}{\# création de l'objet Figure f2}}
...
Tour et Pion (héritant chacun du type Pièce).}
...
\begin{frame}
  ...
  \pause
  \begin{remarque}
    On peut utiliser la fonction built-in \texttt{\emph{classmethod}(methodeClasse)} pour définir la méthode methodeClass() comme méthode de classe.
  \end{remarque}
\end{frame}
...
\small
    \begin{semiverbatim}
		...
		print(Figure.get\_nbfigures()) \pause  \textcolor[rgb]{0,0.58,0}{\# => 2})

...
\begin{frame}
  ...
  \begin{itemize}
  \item Noeud : données\uncover<2->{ + variable pour le chaînage}
  \item<3-> Constructeur (next initialisé à \emph{null})
  \item<4-> Accesseurs ...
  \end{itemize}
  ...
  \begin{exemple}
    \begin{semiverbatim}
      ...
      \hspace{1cm}private int value;

      \uncover<2->{
        \hspace{1cm}private {\color<2>{red}Node} next;

      }
      \uncover<3->{
        \hspace{1cm}public Node(int val) \{

        \hspace{2cm}value = val ; {\color<3>{red}next = null;}
...
\begin{frame}
  ...
  \item avancer tant que l'élément est différent de \emph{null}
  \end{itemize}
  ~\\
...
\end{algorithm2e}
  ...
  \only<2>{
    Parcours d'une liste avec une boucle \emph{for}~:
    \begin{semiverbatim}
...
\end{algorithm2e}
  ...
  \only<2>{
    \begin{semiverbatim}
      public void insertFirst(Node nouv)

      \{

      \hspace{1cm}nouv.setNext(first); 

      \hspace{1cm}first = nouv; 
...
\begin{frame}
  ...
  \begin{tikzpicture}[scale=1,node distance=1.2cm]
    ...
    \node[left=of a] (na) {null};
    \node[right=of c] (nc) {null};
    \draw[flecheb] (a) to (b);
    \draw[flecheb] (b) to (c);
    \draw[flecheb] (b) to (a);
    \draw[flecheb] (c) to (b);
...
\{
  ...
  \hspace{1cm}private Node next;

  \hspace{1cm}private Node prev;
...
\begin{frame}
  ...
  \item Solution : 
    \begin{itemize}
    \item utiliser deux variables de type noeud :

...
\begin{itemize}
	...
	\item Les types simples (sclaires, atomiques) permette de stocker une seule donnée par variable, contrairement aux types composites (containers) comme les séquences et les ensembles,
	\item Les types simples sont immutables (non modifiable)
	\end{itemize}
\end{remarque}
\end{frame}
...
\frametitle{Les séquences immutables}
 Chaîne de caractères:
\begin{itemize}%[<+->]
		\item <1->Elle est délimitée par des apostrophes simple\emph{'} ou triple \emph{'''}, des guillemets simples \emph{"} ou triple \emph{"""} 
	  \item <8->Pour modifier une chaîne, il faut créer une nouvelle.
		\item <12-> Les opérateurs '*' et '+' réalisent la multiplication et le concaténation des chaînes
		\end{itemize}

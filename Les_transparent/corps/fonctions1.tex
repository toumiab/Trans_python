    ...
    \only<3>{
		...
		\begin{itemize}
		  \item Utiliser un nom qui commence par un minuscule\\ 
			Exemple : \texttt{uneFonctionBienNommee}
...
def aj(b, liste=[1,2]):
  ...
  \end{lstlisting}
	\begin{semiverbatim}
	lst=aj(5)   \# \only<1-4>{lst = [1, 2, 5] V/F?}
	\only<2->{lst1=aj(6)  \#} \only<2>{lst1 = ?, lst = ?}\only<3>{lst1 =  [1, 2, 6] V/F?}\only<4>{lst = [1,2,5] V/F?}\only<5>{\textcolor{green}{solution} => lst1 = lst = [1, 2, 5, 6]}
	\end{semiverbatim}
...
%{\small
  \begin{exemple}[Passage par valeur/variable]<4-7>
	...
	print(el)  \textcolor[rgb]{0.24,0.7,0.44}{\# => el = \only<4> {'quatre' V/F? }\only<5-> {'deux' }}
	
	print(lst) \textcolor[rgb]{0.24,0.7,0.44}{\# \only<6-7> {=> lst = ['un', 'deux'] } \only<6>{V/F?}}
	\end{semiverbatim}
	}
   \end{exemple}
...
\begin{frame}[fragile]
  ...
  \begin{exemple}[Passage par variable : type modifiable]
	...
	lst=['un'] ; el='deux'
	ajoutsupp(lst, el)
	print(lst) \textcolor[rgb]{0.24,0.7,0.44}{\# => lst = ['dix','deux'] V/F?}
	ajoutsupp1(lst, el)
	print(lst)  \textcolor[rgb]{0.24,0.7,0.44}{\# => lst = ['douze','six','deux'] V/F? }	}
	\end{semiverbatim}
  \only<2>{
...
\begin{remarques}
	...
	\begin{itemize}
		\item Utilisation du corps principal du programme via la variable \twound{name}\twound{}
...
print(md.cube(2))     # => 8
\end{lstlisting}
Exécution du module  'module\_autre.py' 
\begin{lstlisting}[frame=none]

...
\begin{frame}[fragile]
  ...
  \begin{semiverbatim}
	...
	>>>tab ? , np.shape(tab) ?
\only<2>{tab = erreur V/F? }\only<3>{array([[1, 2, 3],
      [4, 5, 0]] , (2,3) V/F?}\only<4>{array([[1, 2, 3], [4, 5]])?, (2,)?}\only<5>{\textcolor[rgb]{0,0.58,0}{\# solution :} 
...
\begin{frame}[fragile]
  ...
  \begin{semiverbatim}
	...
	>>>import numpy as np
	>>>tab = np.array([1, 2, 3])
	>>>len(tab) , np.shape(tab) ?
\only<1>{3, (1,3) V/F ?}
\only<2>{>>>res = (tab.T == tab) }
\only<2>{>>>res ? erreur - dimension errors V/F?}

\only<3>{\textcolor{red}{Attention : 

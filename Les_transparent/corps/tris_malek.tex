...
\item Les types composites (containers): 
 ...
 \end{itemize}
\pause
\begin{alertblock}{Le type tableau}
  \begin{itemize}
...
\begin{frame}
  ...
  \begin{defi}[Tableau]
    ...
    \begin{itemize}
			...
			\item \verb!=>! Convertir une liste en tableau avec la fonction \emph{array()} de Numpy
		\end{itemize}
   \end{defi}
...
L = np.array([.2, 4 ,5])  \hspace{ 0.1cm}   \textcolor[rgb]{0,0.58,0}{ \# crée un vecteur de 3 réels}

L = np.array(range(10), dtype = np.float) \textcolor[rgb]{0,0.58,0}{\# crée un vecteur }
...
\begin{itemize}
	...
	\item Produit matriciel : \texttt{dot(A,B)}
	exemple : \texttt{np.dot(tab,tab.T)}
...
\begin{frame}
  ...
  \item Taille du tableau : \emph{len(tab}), \emph{len(tab[0])}, \ldots{}
  \end{itemize}
  ...
  \begin{exemple}[Accès aux éléments des tableaux]
    ...
    \begin{semiverbatim}
      ...
      M[1,1] ;  M[1][1]              \textcolor[rgb]{0,0.58,0}{\#  renvoient 7 }
		  ...
		  len(M)  \hspace{ 1.4cm} \textcolor[rgb]{0,0.58,0}{\#  renvoie 2 }
...
\begin{frame}
  \frametitle{Principe du tri par sélection}
  \only<1-9>{
  \begin{itemize}
...
\begin{frame}
  \frametitle{Algorithme du tri par sélection}
{\small
\begin{algorithm2e}[H]
  \caption{triSel}
  \KwIn{tab : entier[]}
